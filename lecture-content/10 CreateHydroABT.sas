...
data WORK.HYDRO_DAILY_STATIONS_XT;
 ...
 set tasks.hydro_daily_stations;
 ...
 WShift_N_A = Water_N - Water_A;
 WShift_B_P = Water_B - Water_P;
 WShift_R_P = Water_R - Water_P;
 WShift_B_I = Water_B - Water_I;
...
/*** dq check only
data firstday;
 set hydro3.Daily_ABT_2020;
 if day(date) = 1;
 keep date WaterLevel;
 run;
***/
...
data WORK.HYDRO_MONTHLY_ABT(where=(YYMM not in ("01JUL1999"d,"01AUG1999"d))); *** Daten unvollständig;
 set WORK.HYDRO_MONTHLY_ABT;
    ...
    format WaterLevelChange 8.3;
    label WaterLevelChange = "Water Level Change in mm";
   run;


proc sort data= WORK.HYDRO_MONTHLY_ABT
           out= WORK.HYDRO_MONTHLY_ABT0;
 by yymm;
run;
